...
class db
{
    ...
    public function addProducer(
        ...
        if ($num_rows > 0) {
            ...
            $sql = "INSERT INTO cocoon (name, birthdate, 
             ...
             zip_code, education, religion, civil_status, name_spouse, farm_participate,
            ...
            if ($resultsql) {
                ...
                foreach ($farm_tool as $tool) {
                    $tool_sql = "INSERT INTO cocoon_farm_tool (cocoon_id, tool_id) 
   ...
   WHERE cocoon_id = '$cocoonID'";
    ...
    {
        $query = "SELECT tool_id FROM cocoon_farm_tool WHERE cocoon_id = '$cocoonID'";
        $result = mysqli_query($this->con, $query);

        $selectedFarmTools = array();

        while ($row = mysqli_fetch_assoc($result)) {
            $selectedFarmTools[] = $row['tool_id'];
        }
    ...
    public function updateCocoonFarmTools($cocoon_id, $farm_tools)
    {
        ...
        foreach ($farm_tools as $action) {
            // Extract the source_id from the array and cast it to integer
            $action_id = (int)$action['tool_id'];
            // Insert the action_id into the database
            $insertSql = "INSERT INTO cocoon_farm_tool (cocoon_id, tool_id) VALUES ($cocoon_id, $action_id)";
            $insertResult = mysqli_query($this->con, $insertSql);
    ...
    public function getSelectedSource($cocoon_id)
    {
        $sql = "SELECT * FROM cocoon_source 
    WHERE cocoon_id = '$cocoon_id'
    ";
        $result = mysqli_query($this->con, $sql);
        return $result;
    ...
    public function getSelectedFarmTool($cocoon_id)
    {
        $sql = "SELECT * FROM cocoon_farm_tool 
    WHERE cocoon_id = '$cocoon_id'
    ";
        $result = mysqli_query($this->con, $sql);
        return $result;
    ...
    public function getPdfFarmTool()
    {
        $sql = "SELECT * FROM
        farm_tool";
        $result = mysqli_query($this->con, $sql);
